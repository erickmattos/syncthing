...
type FakeConnection struct {
	...
	indexUpdates       []protocol.IndexMessage
	clusterConfigs     []protocol.ClusterConfigMessage
	indexReturns       error
	indexUpdateReturns error
}
...
func (f *FakeConnection) IndexUpdate(folder string, files []protocol.FileInfo, flags uint32, options []protocol.Option) error {
	f.indexUpdates = append(f.indexUpdates, protocol.IndexMessage{
		Folder:  folder,
		Files:   files,
		Flags:   flags,
		Options: options,
	})
	return f.indexUpdateReturns
}
